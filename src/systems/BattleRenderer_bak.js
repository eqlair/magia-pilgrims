...
export class BattleRenderer {
    ...
    update() {
        ...
        for (const b of this.engine.bullets) {
            ...
            else if (b.type === 'kick') textureKey = 'hit_effect6';
    ...
    _updateSprite(entity, textureKey) {
        ...
        if (p.visible) {
            ...
            // 向きの変更 (targetEnemyがある場合)
            if (textureKey.startsWith('battle_')) {
                if (entity.hp <= 0 && entity.owner === 'player') {
                    ...
                    // dzがプラス（奥）なら背面(3)、マイナスなら正面(0)
                    // dxがプラス（右）なら右(2)、マイナスなら左(1)  ※一般的に 0:下 1:左 2:右 3:上
                    const angle = Math.atan2(dz, dx) * 180 / Math.PI;
                    let frame = 3; // デフォルトは奥
                    if (angle > 45 && angle < 135) frame = 3; // 奥(背面)
                    else if (angle >= -45 && angle <= 45) frame = 1; // 右（スプライトシート上は1番が右向き）
                    else if (angle >= 135 || angle <= -135) frame = 2; // 左（スプライトシート上は2番が左向き）
                    else frame = 0; // 手前(正面)
            ...
            if (textureKey === 'bullet' || textureKey === 'enemy_bullet' || textureKey === 'grenade' || textureKey === 'hit_effect6' || textureKey.startsWith('weapon_')) {
                ...
                if (textureKey === 'bullet' || textureKey === 'enemy_bullet' || textureKey === 'hit_effect6' || textureKey.startsWith('weapon_')) {
                    if (entity.type && entity.type.startsWith('swing_')) {
                        ...
                        sprite.setAngle(baseAngle + angleRange - (angleRange*2) * progress);
                        // サイズ調整
                        ...
                        if (entity.type === 'swing_004' || entity.type === 'swing_ultimate_004') {
                            ...
                            const startRad = (baseAngle - 90 - angleRange) * Math.PI / 180;
                            const currentRad = (baseAngle - 90 - angleRange + (angleRange*2) * progress) * Math.PI / 180;
                            this.beamGraphics.arc(p.x, p.y, radius, startRad, currentRad, false);
            ...
            if (entity.isDying) {
                ...
                sprite.setAlpha(1.0); // 透明度0（不透明）にする
